...
CREATE TABLE search_terms (
  ...
  needs_scraping BOOLEAN GENERATED ALWAYS AS (
    last_scraped_date IS NULL OR 
    last_scraped_date < NOW() - INTERVAL '7 days'
  ) STORED,
...
CREATE INDEX idx_search_terms_needs_scraping ON search_terms(needs_scraping) WHERE needs_scraping = true;
...
SELECT 
  ...
  END as status,
  EXTRACT(EPOCH FROM (NOW() - last_scraped_date))/86400 as days_since_scraped,
...
BEGIN
  ...
  SELECT 
    ...
    EXTRACT(EPOCH FROM (NOW() - st.last_scraped_date))/86400 as days_since_scraped
...
COMMENT ON COLUMN search_terms.needs_scraping IS 'Automatically true if not scraped in last 7 days';
